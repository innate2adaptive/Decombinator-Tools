...
def getRepositoryPath():
	...
	repopath = ""
	for d in scriptpath.split("/")[1:-1]: 
		if d == "Decombinator":
			# if Decombinator-Tools is inside Decombinator
			repopath += os.sep + d + os.sep + "Decombinator"
			return repopath
		elif d == "Decombinator-Tools":
			# assume Decombinator and Decombinator-Tools share parent directory
			if "Decombinator" in os.listdir(repopath):
				return repopath + os.sep + "Decombinator"
			else:
				print("Error: Could not find Decombinator repository")
		repopath += os.sep + d
	return repopath
...
def demultiplex(test_data_dir):
	...
	print("--------------------------------------------------------------")
	command = ["python",decombinatorpath + os.sep + "Demultiplexor.py","-r1",demultfiles["r1"],
				"-r2",demultfiles["r2"],
				"-i1",demultfiles["i1"],
				"-ix",demultfiles["ind"]]
	execute(command)
...
def decombine():
	...
	for f in fqfiles:
		...
		command = ["python",decombinatorpath + os.sep + "Decombinator.py",
...
def collapse():
	...
	for f in fqfiles:
		...
		command = ["python",decombinatorpath + os.sep + "Collapsinator.py",
...
def translate():
	...
	for f in fqfiles:
		...
		command = ["python",decombinatorpath + os.sep + "CDR3translator.py",
...
if __name__ == '__main__':
	...
	decombinatorpath = getRepositoryPath()
